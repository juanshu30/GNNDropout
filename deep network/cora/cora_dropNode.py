    ...
    def forward(self,TRAIN):
        ...
        if TRAIN == True:
            for i, layer in enumerate(self.hidden_layers):
                ...
                if i % 8 % 4 == 0:
                    y = torch.bernoulli(torch.ones(data.num_nodes,1).to(device)*(1-self.dr)).mul(y)
        else:
            for i, layer in enumerate(self.hidden_layers):
                y = F.relu(layer(y, edge_index))
        y = F.relu(self.fc1(y))
...
np.savetxt("/home/shu30/Research_GraphDropout/AAAI/result_deep/GS_cora_dropN.txt",test_accs)
