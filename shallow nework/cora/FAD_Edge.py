...
class Net(torch.nn.Module):
    ...
    def forward(self,TRAIN,dr=None):
        ...
        if TRAIN == True:
            ...
            y1 = F.relu(self.conv1(y, edges))
            edges = edge_index[:,torch.bernoulli(torch.ones([1,edge_index.shape[1]]).to(device) * (1-self.droprate)).bool().reshape(-1)]
            y = self.conv2(y1, edges) + y1
            y1 = F.relu(y)
            edges = edge_index[:,torch.bernoulli(torch.ones([1,edge_index.shape[1]]).to(device) * (1-self.droprate)).bool().reshape(-1)]
            y = self.conv3(y1, edges) + y1
            y = F.relu(y)
        else:
...
np.savetxt("/home/shu30/Research_GraphDropout/AAAI/result/GS_FAD_Edge_mean.txt",accs_mean)
np.savetxt("/home/shu30/Research_GraphDropout/AAAI/result/GS_FAD_Edge_var.txt",accs_var)