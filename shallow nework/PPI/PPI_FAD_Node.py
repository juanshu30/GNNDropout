...
class Net(torch.nn.Module):
    ...
    def forward(self,data):
        ...
        if TRAIN == True:
            ...
            y = torch.bernoulli(torch.ones(data.num_nodes,self.num_hidden).to(device)*(1-self.droprate)).mul(y1)
            y = self.conv2(y, edge_index) + y1
            y2 = F.relu(y)
            y = torch.bernoulli(torch.ones(data.num_nodes,self.num_hidden).to(device)*(1-self.droprate)).mul(y2)
            y = self.conv3(y, edge_index) + y2
            y = F.relu(y)
        else:
            y1 = F.relu(self.conv1(y, edge_index))
            y2 = F.relu(self.conv2(y1, edge_index) + y1)
            y = F.relu(self.conv3(y2, edge_index) + y2)
        y = F.relu(self.fc1(y))
        y = F.relu(self.fc2(y))
        y = self.fc3(y)
